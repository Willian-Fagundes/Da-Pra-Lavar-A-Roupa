...
def ingest_weather_data(latitude, longitude):
    ...
    daily["score_final"]  = (0.60 * daily["score_base"] + 0.40 * daily["score_base"] * daily["fator_janela"])
    daily["score_final"]  = daily["score_final"].clip(0, 10).round(1)
    daily[["classificacao","cor"]] = daily["score_final"].apply(lambda s: pd.Series(classificar(s)))
